      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          >
            ...
            <h1 className={`text-4xl font-light ${theme.textPrimary}`}>
              {greetingText}, <span className="font-semibold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">{userName}</span>
            </h1>
          </motion.div>
          ...
          >
            <Sparkles className={`w-4 h-4 ${theme.isDark ? 'text-yellow-400' : 'text-yellow-600'}`} />
            <span className={theme.textMuted}>
              {userName !== 'there' ? `${userName}'s Trackr` : 'Your Personal Habit Tracker'}
            </span>
          </motion.div>
        </div>
                      ...
                      <AnimatePresence>
                        {habitMenuOpen === habit.id && (
                          <motion.div
                            ...
                            className={`absolute right-0 top-full mt-2 ${theme.card} py-2 z-20 min-w-[140px] shadow-2xl`}
                          >
                            <button
